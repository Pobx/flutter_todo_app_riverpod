...
class TodoNotifier extends StateNotifier<TodoState> {
  ...
  mapEventsToStates(TodoEvent event) {
    return event.map(todoTitleChanged: (todoTitleChangedEvent) {
      ...
      todolist[
              todolist.indexWhere((element) => element.id == selectedTodo.id)] =
          TodoModel(
              id: selectedTodo.id,
      ...
      todoList.add(TodoModel(
          id: uuid.v1(), title: state.todo.title, isTodoCompleted: false));
      state = state.copyWith(todoList: todoList);
